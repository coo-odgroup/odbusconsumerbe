...
class ListingRepository
{
    ...
    public function busFacilities($busId, $entry_date)
    {
        $bus = $this->bus
            ->with('busContacts')
            ->with([
                'busAmenities' => function ($query) {
                    $query->with([
                        'amenities' => function ($a) {
                            $a->where('status', 1)
                                ->select('id', 'name', 'amenities_image', 'android_image');
                        }
                    ]);
                }
            ])
            ->with([
                'busSafety' => function ($query) {
                    $query->with([
                        'safety' => function ($a) {
                            $a->where('status', 1)
                                ->select('id', 'name', 'safety_image', 'android_image');
                        }
                    ]);
                }
            ])
    ...
    public function getFilterBusList(
        $busOperatorId,
        $busId,
        $busType,
        $seatType,

...
class ReminderBeforeDeboardingJob implements ShouldQueue
{
    ...
    public function __construct($bookingId, $templateKey, $scheduledAt)
    {
...
{
    Log::info("30 Min booking reminder-Executing", [
    ...
    if (!$booking) {
        Log::error("30 Min booking reminder - Booking not found", [
            'booking_id' => $this->bookingId
        ]);
        return;
    }
    ...
    if ((int) $booking->status === 2) {
        Log::info("30 Min booking reminder - Skipped (booking cancelled)", [
            'booking_id' => $this->bookingId
        ]);
        return;
    }
    ...
    if (!$template) {
        Log::error("30 min booking  reminder - Template missing", [
    ...
    Log::info("30-MIN DEBOARDING REMINDER → Notification saved", [
        'booking_id' => $booking->id
    ]);
}
